...
for j in examples:
    ...
    plt.xlabel("Class")
    plt.savefig(path+"SGLD_MNIST_digit_box_"+str(j)+".pdf", format='pdf')
    plt.close()
...
plt.savefig(path+"SGLD_MNIST_first_5_w.pdf", format='pdf')
#plt.show()
plt.close()
...
plt.savefig(path+"SGLD_MNIST_gt.pdf", format='pdf')
#plt.show()
plt.close()
